...
if ($codigoMesa == "admin") {
    ...
    $stmt = $conn->prepare("SELECT codigo_mesa, GROUP_CONCAT(titulo SEPARATOR ', ') AS titulos, SUM(precio * cantidad) AS total, estado_pedido FROM pedidos WHERE codigo_mesa = ? AND TIMESTAMPDIFF(MINUTE, fecha_hora, NOW()) <= 60 GROUP BY codigo_mesa");
